...
export default {
  ...
  data: () => ({
    ...
    emailRules: [
      (v) =>
        !!v || this.lang.FIELD_IS_REQUIRED.replace("field", this.lang.EMAIL),
      (v) =>
        /([a-zA-Z0-9_.-]+)@(.+)[.](.+)/.test(v) || this.lang.WRONG_EMAIL_FORMAT,
      (v) =>
        (!!v &&
          !!v.includes("@") &&
          v.split("@")[0].length >= 1 &&
    ...
    nameRules: [
      (v) =>
        !!v || this.lang.FIELD_IS_REQUIRED.replace("field", this.lang.NAME),
      (v) =>
        v.length >= 2 ||
        this.lang.NAME +
    ...
    additionalInfoRules: [
      (v) =>
        ...
        this.lang.ADDITIONAL_INFO +
          this.lang.FIELD_MUST_BE_LESS_THAN_VALUE.replace("value", 200),
    ],
    alert: {
      visible: false,
      message: "",
  ...
  methods: {
    ...
    onScheduleEvent() {
      ...
      if (this.formValid) {
        try {
          console.log("invitee_name", this.publicEvent.inviteeName);
          console.log("invitee_email", this.publicEvent.inviteeEmail);
          console.log("start_date", this.publicEvent.startDate);
          console.log("timezone", this.publicEvent.timezone);
          /* this.addPublicEvent({
                        invitee_name: ,
                        invitee_email: ,
                        start_date: ,
