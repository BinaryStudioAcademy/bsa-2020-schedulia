  ...
  <VRow class="ma-0 pa-0" v-if="isReady">
    ...
    <VCol v-if="show" sm="12" md="4" lg="3" class="select-time-container col-12 pa-0 mt-0">
      ...
      <div class="time-items-container">
        <div v-for="(time, i) in currentDayAvailableTimes" :key="i" text>
      ...
      if (this.date) {
        for (let date of this.currentTimezoneAvailabilities) {
          if (date.startDate.includes(this.date)) {
            this.currentDayUnavailibilities = date.unavailable.map(time =>
              moment
                .tz(`2010-10-10 ${time}`, this.eventType.timezone)
                .clone()
      ...
      allTimes = allTimes.map(availability => ({
        ...
        endDate:
          ...
          formatTime(availability.startDate)
            ? moment(formatTime(availability.endDate), "YYYY-MM-DD HH:mm:ss")
                .add(1, "days")
                .format("YYYY-MM-DD HH:mm:ss")
